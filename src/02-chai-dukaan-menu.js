...
export function formatChaiMenu(items) {
  ...
  return items.filter(item => typeof item.name === "string" && item.name !== "" && item.price && typeof item.price === "number" && item.price > 0).map(item => `${item.name.toUpperCase()} - Rs.${item.price}`).join(" | ")
}
